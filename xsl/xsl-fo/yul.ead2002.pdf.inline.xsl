...
<xsl:stylesheet version = "1.0" 
  ...
  <xsl:template match="ead:emph | ead:title" mode="inline">
    <xsl:choose>
      <!--MDC:  not sure how best to add this, due to the nature of the style sheets (though I'd prefer to pass a parameter).  However, this will fix the problem
        of emph and title elements in the PDF being output as 9pt font, rather than 11 pt., at the series/subseries levels -->
      <xsl:when test="@render='bold' and ancestor::ead:did/parent::*[@level='subgrp' or @level='series' or @level='subseries' or @otherlevel='accession']">
        <xsl:element name="fo:wrapper" use-attribute-sets="inventory.seriesEtc.unittitle.block">
      ...
      <xsl:when test="@render='italic' and ancestor::ead:did/parent::*[@level='subgrp' or @level='series' or @level='subseries' or @otherlevel='accession']">
        <xsl:element name="fo:wrapper" use-attribute-sets="inventory.seriesEtc.unittitle.block">
